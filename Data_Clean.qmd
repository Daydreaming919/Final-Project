...
Because the Pisa data is at the individual level, while my research is at the country level, I have aggregated the Pisa data. I grouped the data by country for each year and used the student weights provided in the Pisa data set to summarize the students' reading, math, and science scores, along with the other necessary variables. For example, I also calculated the standard deviation, low and high achievers of each score.
...
output_path_all <- here("Data", "pisa_all.csv")
write_csv(pisa_all_years, output_path_all)
...
pisa_aggregated <- pisa_all_years %>%
  ...
  summarise(
    ...
    science_low_achievers = weighted.mean(science <= 409, w = stu_wgt, na.rm = TRUE),
    ...
    science_female = weighted.mean(science[gender %in% c("Female", "1")], 
                                w = stu_wgt[gender %in% c("Female", "1")], na.rm = TRUE),
    science_male   = weighted.mean(science[gender %in% c("Male", "2")], 
                                w = stu_wgt[gender %in% c("Male", "2")], na.rm = TRUE),
...
output_path <- here("Data", "pisa_aggregated.csv")
...
I matched this aggregated data with other country-level macroeconomic data provided by the World Bank, including the key variable Government expenditure on education and other control variables that may be used in the research.
```{r}
library(janitor)
...
pisa_data <- read_csv(here("Data", "pisa_aggregated.csv"), show_col_types = FALSE) %>%
  mutate(
    year = as.integer(year),
    iso3c = case_when(
      country == "KSV" ~ "XKX",
...
final_dataset <- pisa_data %>%
  ...
  mutate(
    edu_exp_capita = (as.numeric(edu_exp) / 100) * as.numeric(gdp_ppp),
  )
write_csv(final_dataset, here("Data", "final_dataset.csv"))
